...
// map initialization (Leaflet.js plugin)
    if ($("#map")[0]) {	// uinitialize if map container exists
        ...
        var geoDataUrlBase = "/lots/lots-json"
                , geoDataUrl
                ;
    ...
    $("#city").autoComplete({
        ...
        source: function (term, response) {
            $.getJSON('/localities/localities-autocomplete', {city: term}, function (data) {
                cities = []; // масив міст
                ids = [];	// масив id міст
...
// map in lot page
    var $sellerMap = $("#map.seller__map")
        ...
        , lotIcon = L.divIcon({// сірий маркер
            ...
            iconAnchor: [17.5, 39], //making arrow points ti a place, not top left corner of div
            popupAnchor: [0, -37],
            html: "<div class='arrowBox__circle circle_" + "red" + "'></div>"	// circle inside the marker
        });
