...
// изменение некоторых настроек юзера
    $('#form_contacts').submit(function(e){
        e.preventDefault();    
        var data = $('#form_contacts').serialize();
        $.ajax({
            type: 'post',
            data: data,
            url : '/feedback/create-contact-request',
            cache: false,
            dataType: 'json',
            success: function(response){
                $.arcticmodal('close');
    ...
    // ограничение для вывода заголовков постов и прочих подобных вещей
    function twoLinesTitle(selector){
        $(selector).dotdotdot({
            ellipsis: "... ",
            wrap: "letter",
