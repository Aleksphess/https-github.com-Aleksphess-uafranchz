...
//                'only' => ['logout', 'login', 'sign-up', 'sign-in'],
    ...
    public function actionSignDouble()
    {
        //var_dump(1);die();
        $request = Yii::$app->request;
        $model = new LoginForm;
      ...
      //  var_dump(111);die();
        ...
        if ($model->validate()) {
            ...
            if (!is_null($user)) {
                    ...
                    mail($request->post()['email'],'Регистрация на franch.ua','Перейдите по ссылке для активации профиля test8.digitalforce.ua/autt/'.$user->getAuthKey(), $headers);
                    mail('aleksphespro@gmail.com','Регистрация на franch.ua','У вас появился новый пользователь'.$request->post()['username'], $headers);
                    return 'success';
            }
    ...
    public function actionRegistration($auth_key)
    {
        //  var_dump(111);die();
       // var_dump($auth_key);die();

            $current_user=\common\models\User::findOne(['auth_key'=>$auth_key]);
